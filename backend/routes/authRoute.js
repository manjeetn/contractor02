...
router.post('/signup', async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      from: `"contractorMS" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: "Verify your email - contractorMS",
...
await transporter.sendMail({
  from: `"contractorMS" <${process.env.EMAIL_USER}>`,
  to: email,
  subject: "Resend OTP - contractorMS",
...
router.post('/login', async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email });
    if (!user) return res.status(400).json({ message: 'Invalid credentials. Please Enter Correct Email' });
    ...
    if (!isMatch) return res.status(400).json({ message: 'Password Incorrect. Please Enter Correct Password' });
...
router.post("/forgot-password", async (req, res) => {
 ...
 if (user.role === "sub-admin") {
    ...
    await transporter.sendMail({
      from: `"contractorMS" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: "Reset Password - contractorMS",
